...
// Patient on video call; AI provides real-time correction (angle, speed, range); therapist focuses on coaching not form checking
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / cf-telehealth-live-feedback',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_telehealth_live_feedback`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-telehealth-live-feedback feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Telehealth live feedback'.
Context: Patient on video call; AI provides real-time correction (angle, speed, range); therapist focuses on coaching not form checking
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-telehealth-live-feedback/run error:', err);
