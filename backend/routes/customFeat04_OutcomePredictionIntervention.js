...
// Early-stage prediction of poor outcomes; AI suggests therapy plan adjustments or referral to physician for further workup
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / cf-outcome-prediction-intervention',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_outcome_prediction_intervention`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-outcome-prediction-intervention feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Outcome prediction + intervention'.
Context: Early-stage prediction of poor outcomes; AI suggests therapy plan adjustments or referral to physician for further workup
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-outcome-prediction-intervention/run error:', err);
