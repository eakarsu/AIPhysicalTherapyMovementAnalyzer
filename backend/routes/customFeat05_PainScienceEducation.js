...
// AI generates patient-specific pain science education videos based on diagnosis (e.g., if arthritis: explain cartilage, exercise benefits, realistic expectations)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / cf-pain-science-education',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_pain_science_education`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-pain-science-education feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Pain science education'.
Context: AI generates patient-specific pain science education videos based on diagnosis (e.g., if arthritis: explain cartilage, exercise benefits, realistic expectations)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-pain-science-education/run error:', err);
