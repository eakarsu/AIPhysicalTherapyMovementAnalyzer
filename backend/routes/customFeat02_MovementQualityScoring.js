...
// Computer vision rates movement against expected ROM/quality standards; tracks improvement over weeks; auto-generates progress reports with visual comparisons
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / cf-movement-quality-scoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_movement_quality_scoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-movement-quality-scoring feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Movement quality scoring'.
Context: Computer vision rates movement against expected ROM/quality standards; tracks improvement over weeks; auto-generates progress reports with visual comparisons
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-movement-quality-scoring/run error:', err);
