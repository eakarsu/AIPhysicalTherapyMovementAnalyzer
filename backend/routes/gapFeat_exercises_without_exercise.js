...
// Gap Feature: Exercises without \'/exercise
// Exercises without /exercise-difficulty-adapt (adjust difficulty based on performance)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / gap-exercises-without-exercise',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Exercises without \'/exercise'.\nProject: AIPhysicalTherapyMovementAnalyzer.\nContext: Exercises without /exercise-difficulty-adapt (adjust difficulty based on performance)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['exercises-without-exercise', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'exercises-without-exercise', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
