...
// Patient performs HEP at home; video streams to cloud; AI analyzes form in real-time, sends corrective feedback via mobile app; therapist reviews weekly; AI predicts compliance and notifies therapist i
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / cf-agentic-hep-execution',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_hep_execution`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-hep-execution feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic HEP execution'.
Context: Patient performs HEP at home; video streams to cloud; AI analyzes form in real-time, sends corrective feedback via mobile app; therapist reviews weekly; AI predicts compliance and notifies therapist if patient falling behind
Project: AIPhysicalTherapyMovementAnalyzer.
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-hep-execution/run error:', err);
