...
// Gap Feature: Limited EHR/medical records integration (only stub)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPhysicalTherapyMovementAnalyzer / gap-limited-ehr-medical-records-integration-only-stub',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited EHR/medical records integration (only stub)'.\nProject: AIPhysicalTherapyMovementAnalyzer.\nContext: Limited EHR/medical records integration (only stub)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-ehr-medical-records-integration-only-stub', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-ehr-medical-records-integration-only-stub', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
